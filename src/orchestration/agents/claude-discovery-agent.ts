...
Analyze the codebase now and return only the JSON, no additional explanation.`;
      ...
      if (environment.application.runtime_detected.length === 0) {
        ClaudeHelper.formatProblemSolution(
          'No LLM runtime detected in your infrastructure',
          'Consider adding OpenAI, Anthropic, or HuggingFace libraries to enable LLM optimization',
          {
      ...
      if (environment.infrastructure.gpu_inventory.length === 0) {
        ClaudeHelper.formatProblemSolution(
          ...
          'Add GPU infrastructure (NVIDIA A100/H100) for 10-50x inference speedup',
          {
   ...
   */
  private suggestSolutions(environment: EnvironmentProfile): Array<{title: string; description: string; savings?: number; effort?: string}> {
    const solutions: Array<{title: string; description: string; savings?: number; effort?: string}> = [];
   ...
   */
  private async fallbackDiscovery(environment: EnvironmentProfile): Promise<EnvironmentProfile> {

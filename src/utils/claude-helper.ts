...
export class ClaudeHelper {
   ...
   */
  static async ensureApiKey(): Promise<void> {
    if (this.checkApiKey()) {
  ...
  static formatAnalysis(title: string, analysis: any): void {
    ...
    if (analysis.problems && analysis.problems.length > 0) {
      ...
      analysis.problems.forEach((problem: string, i: number) => {
    ...
    if (analysis.findings && analysis.findings.length > 0) {
      ...
      analysis.findings.forEach((finding: string, i: number) => {
    ...
    if (analysis.solutions && analysis.solutions.length > 0) {
      ...
      analysis.solutions.forEach((solution: any, i: number) => {
   ...
   */
  static showRecommendation(title: string, description: string, priority: 'high' | 'medium' | 'low' = 'medium'): void {
    const colors = {
      high: chalk.red,
      medium: chalk.yellow,
      low: chalk.green

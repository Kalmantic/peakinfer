 ...
 */
export async function storePRState(
  octokit: OctokitType,
  ...
  const stateComment = comments.data.find(c =>
    c.body?.startsWith(STATE_COMMENT_MARKER)
  );
...
export async function loadPRState(
  ...
  try {
    ...
    return JSON.parse(json) as PRState;
  } catch {
    return null;
  }
}
 ...
 */
export async function handleFixAll(
  octokit: OctokitType,
  context: typeof github.context
): Promise<{ success: boolean; message: string }> {
  const prNumber = context.payload.issue?.number;
  ...
  const pendingWithFixes = state.issues.filter(
    i => i.status === 'pending' && i.suggestedFix && i.location
  );

 ...
 */
function formatLocation(location, repoContext) {
    if (!location)
        return '-';
 ...
 */
function getVerdictText(critical, warnings) {
    if (critical >= 2) {
...
export function generatePRComment(data) {
    ...
    if (newIssues.length > 0) {
        ...
        if (criticalIssues.length > 0)
            parts.push(`${criticalIssues.length} critical`);
 ...
 */
export function generateExhaustedComment(used, limit, unanalyzedCount) {
    const lines = [];
    lines.push('## PeakInfer Analysis\n');

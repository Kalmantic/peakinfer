...
export class RunnerEvaluatorAgent {
     ...
     */
    async executeWithEarlyStopping(plan, environment, workloadProfile, dryRun) {
        ...
        for (const templateId of plan.execution_order) {
            ...
            if (this.shouldStopEarly(results, plan.early_stopping_criteria, startTime, cumulativeSavings, candidatesTested)) {
            ...
            try {
                // Execute template
                const result = await this.executionAgent.executeTemplate(template, environment, { dryRun });
                results.push(result);
     ...
     */
    shouldStopEarly(results, criteria, startTime, cumulativeSavings, candidatesTested) {
        ...
        if (results.length >= 3) {
            ...
            const recentSavings = recentResults.reduce((sum, r) => sum + (r.cost_savings || 0), 0);
     ...
     */
    async evaluateCandidate(template, environment, workloadProfile, sampleSize) {

...
export class WorkloadProfilerAgent {
     ...
     */
    async profileWorkload(workloadDataPath, environment) {
     ...
     */
    async loadWorkloadData(workloadDataPath) {
        if (!workloadDataPath) {
...
Return only the JSON, no explanation.`;
     ...
     */
    async generateRepresentativeSamples(clusters) {
        const samples = [];
     ...
     */
    calculateCostBreakdown(events, clusters) {
        const breakdown = {};
        for (const cluster of clusters) {
     ...
     */
    createSyntheticProfile(environment) {
        const clusters = [
            {
        ...
        const samples = clusters.flatMap(cluster => cluster.representative_prompts.map(prompt => ({
            intent: cluster.intent_name,
            prompt,
            expected_output_length: cluster.avg_tokens,
            frequency: cluster.sample_count

...
#### GitHub UX Design (v2.0 - CodeRabbit Style)

**Design Principles** (Julie Zhou behavior-first):
- **5-second decision**: User knows instantly if PR needs attention
- **30-second action**: Batch commit all fixes with 2 clicks
- **Native GitHub UX**: Uses `suggestion` blocks for native buttons
- **AI-ready**: "Prompt for AI Agents" for Copilot/Claude
- **Progressive disclosure**: Commits, Files, Fixes use `<details>`
- **No color reliance**: Text labels "Critical", "Warning"

**Verdict Logic:**
```
≥2 critical        → Review required
1 critical OR >5   → Review recommended
1-5 warnings       → Safe to merge (with improvements)
0 issues           → Safe to merge
```

---

### 2.5. GitHub UX - Full Screen Flow
